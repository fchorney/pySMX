...
class PackedSensorSettings(object):
    ...
    def to_array(self) -> list[int]:
        return [
            self.load_cell_low_threshold,
            self.load_cell_high_threshold,
            *self.fsr_low_threshold,
            *self.fsr_high_threshold,
            self.combined_low_threshold,
            self.combined_high_threshold,
            self.reserved,
        ]
...
class SMXStageConfig(object):
    ...
    def to_packed_bytes(self) -> bytes:
        flat_panel_settings = []
        for sublist in [x.to_array() for x in self.panel_settings]:
            flat_panel_settings.extend(sublist)

        # On a write we add the size of the packet as the first byte
        write_struct = self.STRUCT_FMT[0] + "B" + self.STRUCT_FMT[1:]
        ...
        return struct.pack(
            ...
            self.auto_calibration_max_deviation,
            self.bad_sensor_minimum_delay_seconds,
            self.auto_calibration_averages_per_update,
            self.auto_calibration_samples_per_average,
            self.auto_calibration_max_tare,
            *self.enabled_sensors,
            self.auto_lights_timeout,
            *self.step_color,
            *self.platform_strip_color,
            self.auto_light_panel_mask,
            self.panel_rotation,
            *flat_panel_settings,
            self.pre_details_delay_milliseconds,
            *self.padding,
        )
    ...
    def from_packed_bytes(cls, data: bytes) -> "SMXStageConfig":
        ...
        packed_sensor_settings = [
            PackedSensorSettings.from_unpacked_values(p_sensor_data[i : i + sensor_value_count])
